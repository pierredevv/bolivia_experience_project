...
describe('RecommendationsService', () => {
  ...
  describe('getPersonalized', () => {
    it('should return personalized recommendations based on favorites', async () => {
      ...
      mockPrisma.review.findMany.mockResolvedValue([
        { placeId: 'p3', rating: 5, place: { categoryId: 'cat-2', ratingAvg: 4.5 } },
      ]);
      mockPrisma.searchHistory.findMany.mockResolvedValue([]);
      mockPrisma.place.findMany.mockResolvedValue([
        { id: 'p4', category: { id: 'cat-1', name: 'Restaurantes', icon: '🍽️' }, photos: [] },
      ]);
  ...
  describe('getTrending', () => {
    it('should return trending places', async () => {
      const mockPlaces = [
        { id: 'p1', name: 'Lugar Trending', category: { id: 'cat-1', name: 'Restaurantes' }, photos: [], _count: { reviews: 10, favorites: 5 } },
      ];
  ...
  describe('getSimilar', () => {
    it('should return similar places by category', async () => {
      ...
      mockPrisma.place.findMany.mockResolvedValue([
        { id: 'p-similar', category: { id: 'cat-1', name: 'Restaurantes' }, photos: [] },
      ]);
