...
describe('ReferralsService', () => {
  ...
  describe('applyReferralCode', () => {
    ...
    it('should throw BadRequestException for invalid code', async () => {
      ...
      await expect(service.applyReferralCode('user-2', 'INVALID')).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException when using own code', async () => {
      mockPrisma.referral.findUnique.mockResolvedValue({ userId: 'user-1', code: 'OWNCODE' });

      await expect(service.applyReferralCode('user-1', 'OWNCODE')).rejects.toThrow(BadRequestException);
    });

    it('should throw ConflictException if user already used a referral code', async () => {
      mockPrisma.referral.findUnique.mockResolvedValue({ userId: 'referrer-1', code: 'CODE1' });
      mockPrisma.referralUse.findUnique.mockResolvedValue({ referredUserId: 'user-2' });

      await expect(service.applyReferralCode('user-2', 'CODE1')).rejects.toThrow(ConflictException);
    });
  });
