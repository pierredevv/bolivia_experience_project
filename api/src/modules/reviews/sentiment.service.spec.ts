...
describe('SentimentService', () => {
  ...
  describe('analyzeSentiment', () => {
    it('should detect positive text', () => {
      const result = service.analyzeSentiment('Excelente lugar, muy recomendado, Increíble servicio');
      expect(result.score).toBeGreaterThan(0);
      expect(result.label).toBe('positive');
    });

    it('should detect negative text', () => {
      const result = service.analyzeSentiment('Terrible experiencia, muy malo y horrible lugar');
      expect(result.score).toBeLessThan(0);
      expect(result.label).toBe('negative');
    });

    it('should detect neutral text', () => {
      const result = service.analyzeSentiment('El lugar está ahí, es un espacio');
    ...
    it('should handle mixed sentiment', () => {
      const result = service.analyzeSentiment('El lugar es bueno pero el servicio es malo');
      expect(result.label).toBe('neutral');
      expect(result.score).toBe(0);
    });
  });
