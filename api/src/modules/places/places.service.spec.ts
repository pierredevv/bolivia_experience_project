...
describe('PlacesService', () => {
  ...
  describe('findAll', () => {
    it('should return paginated active places by default', async () => {
      const mockPlaces = [
        {
          id: 'place-1',
          name: 'Test Place',
    ...
    it('should filter by isActive=false (hidden places)', async () => {
      ...
      const result = await service.findAll({ skip: 0, limit: 20, isActive: false });

      expect(result.data).toEqual(hiddenPlaces);
      expect(mockPrisma.place.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
        ...
        expect.objectContaining({
          where: expect.objectContaining({
            OR: expect.arrayContaining([
              expect.objectContaining({ name: expect.objectContaining({ contains: 'restaurant' }) }),
            ]),
          }),
        })
      );
  ...
  describe('toggleStatus', () => {
    it('should toggle isActive from true to false', async () => {
      ...
      mockPrisma.place.findUnique.mockResolvedValue(mockPlace);
      mockPrisma.place.update.mockResolvedValue({ ...mockPlace, isActive: false });
    ...
    it('should throw NotFoundException for invalid id', async () => {
      ...
      await expect(service.toggleStatus('invalid-id')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('remove', () => {
    it('should delete place', async () => {
      ...
      expect(result).toEqual(mockPlace);
      expect(mockPrisma.place.delete).toHaveBeenCalledWith({ where: { id: 'place-1' } });
    });
  ...
  describe('addPhoto', () => {
    it('should add photo with correct displayOrder', async () => {
      mockPrisma.placePhoto.aggregate.mockResolvedValue({ _max: { displayOrder: 3 } });
      ...
      const result = await service.addPhoto('place-1', 'https://example.com/photo.jpg');
