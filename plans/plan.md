...
**Verificación (232/232 tests API + build OK, 50/50 tests Flutter + analyze OK, web tsc OK + 46/46 vitest)**: campos `status` (`@default("pending")`), `organizer` y `price` agregados al modelo `Event` en los 3 schemas (`schema.sqlite.prisma` `model Event` L213-234; postgres con `@db.VarChar(20)`/`@db.VarChar(255)`/`@db.Decimal(10,2)`); `prisma validate` OK ×2 (+postgres solo ambiental) + gen + `db push` a `dev.db`. Seed: los eventos sembrados se pasan a `approved` vía `updateMany` (`api/prisma/seed.ts`) para seguir visibles. `events.service.ts`: las lecturas públicas filtran `{ isActive: true, status: "approved" }` (`findAll` L62, `findToday` L90, `findById` L98-106); `create` fuerza `pending + isActive:false` (L108-116); `update`/`remove` pasan por `findByIdRaw` (L51-57, lanza NotFound si no existe); nuevos `findAllAdmin` (filtro `status` sin `isActive`, L15-38) y `updateStatus` (approved → `isActive:true`, resto `false`, L40-49). `events.controller.ts`: `POST /events` con solo `@UseGuards(JwtAuthGuard, RolesGuard)` sin `@Roles("admin")` (L68-75); PUT/DELETE siguen admin-only. Nuevo `events.admin.controller.ts` (`@Controller("admin/events")`, todo `@Roles("admin")`): `GET /admin/events?status&page&limit` y `PATCH /admin/events/:id/status` con `UpdateEventStatusDto` (`@IsIn(["pending","approved","rejected"])`, L22-25); registrado en `events.module.ts`. App Flutter: `Event` model con `status/organizer/price` + `createEvent` (POST /events) en `events_service.dart`; pantalla `create_event_screen.dart` (nombre, ubicación, fecha/hora inicio+fin, toggle gratuito/pago → precio en Bs., opcionales descripción/organizador/tipo de evento); ruta protegida `/events/create` en `router.dart` (antes de `/events/:id`) + FAB "Crear" en `events_screen.dart`. Web admin: `Events.tsx` lista vía `GET /admin/events` con tabs de filtro (todos/pendientes/aprobados/rechazados), badge de estado, botones Aprobar/Rechazar (ConfirmDialog) → `PATCH /admin/events/:id/status`, y form con organizador/precio; `eventsApi.adminGetAll`/`updateStatus` en `api.ts`; hooks `useAdminEvents`/`useUpdateEventStatus` en `useEvents.ts`. `events.service.spec.ts` extendido (+5 tests: servicios de `findById`, `create` pending/inactivo, `findAllAdmin`, `updateStatus`).
