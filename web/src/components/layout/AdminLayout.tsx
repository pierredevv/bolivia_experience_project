...
const navGroups = [
  { label: 'Principal', items: navigation.slice(0, 1) },
  { label: 'Gestión', items: navigation.slice(1, 6) },
  { label: 'Contenido', items: navigation.slice(6, 8) },
  { label: 'Sistema', items: navigation.slice(8) },
]
...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 text-white selection:bg-emerald-500/20">
      ...
      {/* ── Brand: Official Logo Asset ── */}
      <div className="flex flex-col items-start px-6 pt-6 pb-2 flex-shrink-0">
        <img
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-500 mt-8 mb-1">
          Admin Panel Global
        </p>
      </div>

      {/* ── Grouped Nav ── */}
      <nav className="flex-1 px-4 space-y-5 overflow-y-auto pb-4 mt-2">
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className="px-4 mb-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-600">
              {group.label}
            </p>
            <div className="space-y-1">
              {group.items.map((item) => (
                <NavLink
                  ...
                  onClick={onLinkClick}
                  className={({ isActive }) =>
                    `group flex items-center gap-4 px-4 py-3.5 rounded-2xl text-base font-bold transition-all duration-300 ${
                      isActive
              ...
              <div className="relative">
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 text-slate-400 transition-transform duration-300 flex-shrink-0 ${
                      userMenuOpen ? 'rotate-180 text-slate-900' : ''
                    }`}
                  />
                </button>
                ...
                {userMenuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl shadow-slate-950/10 dark:shadow-slate-950/40 border border-slate-100 dark:border-slate-800 z-50 overflow-hidden">
